...
static MYSQL_SYSVAR_STR(hostname, hostname,
  PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
  "The hostname of the shamir server.",
  NULL, NULL, "");

static MYSQL_SYSVAR_UINT(port, port,
                         PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_READONLY,
                        "The port number of the shamir server.",
                        NULL, NULL, 9999, 0, 65535, 0);
...
std::unordered_map<unsigned int,keyentry> keys;
...
struct st_mariadb_encryption remote_key_management_plugin= {
...
static int remote_key_management_plugin_init(void *p)
{
  try
  {
    SSLClient client(hostname, port, filekey);
    keys = client.GetKeys();
    return 0;
  } catch (const std::runtime_error& error)
  {
    my_printf_error(EE_READ, error.what(), ME_ERROR_LOG);
    return 1;
  }
}

static int remote_key_management_plugin_deinit(void *p)
{
  keys.clear();
  return 0;
}
...
maria_declare_plugin(file_key_management)
{
  ...
  &remote_key_management_plugin,
  "remote_key_management",
  "Stefano Secondo",
  "Remote key management plugin",
  PLUGIN_LICENSE_GPL,
  remote_key_management_plugin_init,
  remote_key_management_plugin_deinit,
  0x0100 /* 1.0 */,
